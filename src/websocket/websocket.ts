...
export default class PulseWebSocket {
  ...
  private open = (accountId: number): void => {
    ...
    const wsUrl = "wss://localhost:5000/api/v1/stream?account_id=" + accountId;
  ...
  private newMessage = (message: any): void => {
    storage.getMany(["account_id", "hash", "salt"], (error, result) => {
      ...
      if (message.type === 0) {
        ...
        const conversationUrl = "https://localhost:5000/api/v1/conversations/" +
    ...
    setTimeout(() => {
      storage.get("account_id", (error: any, id: string) => {
        if (!error && id.length !== 0) {
          const urlToGet = "https://localhost:5000/api/v1/conversations/unread_count?account_id=" + id;
